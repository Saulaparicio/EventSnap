...
export default function EventGalleryManager({ event, initialPhotos }: Props) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 w-full">
      ...
      {/* SideNavBar (Admin Moderation Context) */}
      <aside className="hidden lg:flex flex-col w-60 shrink-0 gap-6 border-r border-[#e5e3dc] pr-6 min-h-[calc(100vh-12rem)] justify-between">
        <div className="space-y-6">
          <div className="flex items-center gap-3 px-2">
            <div className="h-9 w-9 bg-black rounded-xl flex items-center justify-center text-white">
              <span className="material-symbols-outlined text-md animate-none">celebration</span>
            </div>
            <div>
              <h3 className="text-xs font-bold text-black uppercase tracking-wider">EventSnap Admin</h3>
              <p className="text-[10px] text-[#76777d] uppercase tracking-widest font-semibold mt-0.5">Manage live feeds</p>
            </div>
          </div>
          <nav className="flex flex-col gap-1">
            <Link
              href="/dashboard"
              className="flex items-center gap-3 px-3 py-2 text-[#45464d] hover:bg-[#f6f3f5] rounded-xl transition-all text-xs font-semibold"
            >
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-3 px-3 py-2 w-full text-left rounded-xl transition-all text-xs cursor-pointer",
                currentMainTab === 'moderation' 
                  ? "bg-[#86f2e4] text-[#006f66] font-bold shadow-sm" 
                  : "text-[#45464d] hover:bg-[#f6f3f5] font-semibold"
              )}
            >
            ...
            <button
              className="flex items-center gap-3 px-3 py-2 w-full text-left text-[#45464d] hover:bg-[#f6f3f5] rounded-xl transition-all text-xs font-semibold cursor-not-allowed"
              disabled
            >
        ...
        {/* Storage card exactly matching Google Stitch */}
        <div className="space-y-4">
          <div className="bg-[#fcf8fa] border border-[#e5e3dc] p-4 rounded-2xl shadow-sm">
            <p className="text-[10px] font-bold text-black mb-1 uppercase tracking-wider">Storage Usage</p>
            <div className="w-full bg-[#f0edef] h-1.5 rounded-full overflow-hidden mb-2 mt-2">
              <div className="bg-[#006a61] h-full" style={{ width: `${storagePercent}%` }}></div>
            </div>
            <div className="flex justify-between text-[10px] text-[#76777d] font-semibold">
              <span>{storageUsedMb.toFixed(0)} MB used</span>
              <span>{storagePercent}%</span>
            </div>
            <button className="w-full mt-4 py-2 bg-black text-white hover:bg-slate-800 rounded-xl text-xs font-bold transition-all active:scale-95 cursor-pointer">
              Upgrade Plan
            </button>
          </div>
        </div>
